...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        var __ALLDAYEVENTTEMP = '<div class="rb-o ${eclass}" id="${id}" title="${title}" style="color:${color};"><div class="dhdV" style="display:none">${data}</div><div class="${extendClass} rb-m" style="background-color:${color};background:rgb(0,176,255)">${extendHTML}<div class="rb-i" style="color:white">${content}</div></div></div>';
        ...
        function populate() {
            ...
            if (option.url && option.url != "") {
				...
				console.log(option.url + '&curdate=' + param[0].value);
                
                $.ajax({
                    type: option.method, //
                    url: option.url + '&curdate=' + param[0].value,
                    data: param,				   
        ...
        function dayshow(e, data) {
            ...
            */
            ...
            var course_id = $(this).attr('id').match(/(\d+)/g);
