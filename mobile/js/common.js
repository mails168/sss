...
function login(){
    ...
    myApp.yywLogin(false,'一键登录（免注册）',function(username, password){
      ...
      var loginBtn = $('.login-box .modal-button:nth-child(2)').attr('disabled', "true").addClass('inactive');
        ...
        if(isWeiXin() && $('.login-wechat').length == false ){
            $('.modal-button:nth-child(1)').after('<span class="login-wechat" onclick="javascript:location.href='+"'/auth'"+'" >微信</span>');
        }
...
function new_add_collect(product_id,product_type,b){
    ...
    $$.ajax({
        ...
        success:function(result){
            ...
            if (result.err) {return false};
            $$(b).find('em').addClass('follow-light');
            // TODO 显示收藏成功。
            // 
        }
    });
